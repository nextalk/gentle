...
db.optionsToSql = function( options, isUpdate ){
	options = options || {};
	var res = {}
		...
		for( var k in conditions ) {
			...
			if( typeof val === "string" ) {
				...
				if( Array.isArray(val[0]) ) {
					...
					sep = " ";
					val = "BETWEEN "+mysql.escape(val[0])+" AND " + mysql.escape(val[1]);
				}
			} else if( val === null ) {
				sep = " IS ";
				val = "NULL";
	...
	if(page && parseInt(page.per_size)) {
		...
		res.page = "LIMIT 10 OFFSET 0";
	}
	var sql = [];
	if( res.conditions ){
		sql.push(res.conditions);
	}
	if( !isUpdate && res.sort ){
		sql.push(res.sort);
	}
	if( !isUpdate && res.page ){
		sql.push(res.page);
	}
	res.sql = sql.join(" ");
	return res;
...
db.all		= function(tableName, options, fn){
	...
	return this.query("SELECT * FROM ?? " + options.sql, [tableName], fn);
}

db.count	= function(tableName, options, fn){
	options = this.optionsToSql(options);
	return this.query("SELECT count(*) num FROM ?? " + options.sql, [tableName], function(err, rows){
		fn && fn(err, err ? rows : (rows[0] && rows[0].num));
	});
}
...
db.find		= function(tableName, options, fn){
	...
	options = this.optionsToSql(options);
	return this.query("SELECT * FROM ?? " + options.sql, [tableName], function(err, rows){
