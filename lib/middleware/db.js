...
db.db = function _db(req, res, next){
	req.db = req.app.db.instance();
	res.locals.db = req.db;
	next();
};

db.per = function _per(req, res, next){
	var db = req.db;
	if(db.per)
		db.per(req, res, next);
	else
...
var renameHelper = {
	"byDate": function byDate(filename, ext){
		var name = dateFormat(new Date(), "yyyy-mm-dd/HHMMss") + parseInt(Math.random()*100000) + ext;
		return name;
	}
};
		...
		Object.keys(req.files || {}).forEach(function(name){
			if( req.files[name] && table.hasColumn(name) ) {
				...
				if(col.type == "image" || col.type == "file"){
					...
					var rename = col.extra && col.extra.rename;
					rename = rename && ( typeof rename == "function" ? rename : renameHelper[rename] );
					if(rename){
						var ext = val.split(".");
						ext = ext.length > 1 ? ("." + ext.slice(-1)[0]): "";
						filename = ext ? filename.replace(ext, "") : filename;
						filename = val = rename(filename, ext);
					...
					if(col.type == "image" && col.extra){
						...
						if( box ){
							//Thumb image
							var extent = col.extra.extent;
							(Array.isArray(box) ? 
								(Array.isArray(box[0]) ? box : [box]) 
								: [[box, box]]).forEach(function(box){
									var d = path.join(dir, "" + box[0] + "x" + (box[1] || box[0]));
									mkdirp.sync(path.dirname(path.join(d, filename)));
									var img = gm(origin), width = box[0], height = box[1] || box[0];
									img.size(function(err, val){
										if(val && val.height && extent && width == height){
											var e = Math.max(val.height, val.width);
											img.gravity("Center").extent(e, e);
											if( extent !== true )
												img.background(extent);
										}
										img.thumb(width, height, path.join(d, filename), box[2] || 100, function(err){
											//TODO: fix error
...
db.action = function _action(req, res, next, action){
	...
	if(req.params.id){
		...
		action = req.action = req.table.action(action);
	}else{
		//Multi
		if(!req.table.hasMultiAction(action)){
			return next();
		}
		action = req.action = req.table.multiAction(action);
	}
