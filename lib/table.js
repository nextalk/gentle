...
table.__defineGetter__("db", function(){
	return this.parent;
});
	...
	page(function(err){
		...
		self.db.all(self.name, cond, function(err, rows){
			...
			self._associateData(rows || [], function(err){
				if(err)
					return fn && fn(err);
				self._selectData(cond.conditions, function(err){
					fn && fn(err, rows);
				});
			});
		});
...
table._fillDefaultData = function(type, obj, cb){
	obj = obj || {};
	async.eachSeries(this.columns(), function(col, done){
		var value = col.config(type);
		if(typeof value == "function"){
			if( value.length > 2 ) {
				//Async
				value.call(null,obj[col.name], obj, function(err, value){
					if(value !== undefined){
						obj[col.name] = value;
					}
					return done(err);
				...
				value = value.call(null,obj[col.name], obj);
				if(value !== undefined){
					obj[col.name] = value;
				}
			}
		}else if(obj[col.name] === undefined && value !== undefined){
			obj[col.name] = value;
		}
		done();
	}, function(err){
...
table.new = function(params, fn){
	var self = this;
	this._fillDefaultData("value", extend({}, params, this.conditions()), function(err, obj){
		if(err)
			return fn && fn(err, obj);
		self._selectData(obj, function(err){
			fn && fn(err, obj);
		});
	});
	return this;
...
table.find = function(id, fn, optionForFilter){
	...
	this.db.find(this.name, op, function(err, row){
		if(err)
			return fn && fn(err);
		self._selectData(extend(row, optionForFilter), function(err){
			fn && fn(err, row);
		});
	});
	return this;
...
table.update = function(id, obj, fn, checkPermit){
	...
	//Note that update multiple ids
	var self = this;
	self._fillDefaultData("update", data, function(err, data){
		if(err)
			return fn && fn(err);
		if(Object.keys(data).length){
			self.db.update(self.name, op, data, function(err, row){
				fn && fn(err, row);
			});
		}else{
...
table.create = function(obj, fn, checkPermit){
	...
	var self = this;
	self._fillDefaultData("create", data, function(err, data){
		if(err)
			return fn && fn(err);
		if(Object.keys(data).length){
			self.db.create(self.name, data, function(err, row){
				fn && fn(err, row);
			});
		}else{
			fn && fn(new Error("Nothing for create"));
		}
	});
	return this;
}
...
/**
 * Fetch data for the select column
 *
 * @api private
 */
table._selectData = function(params, fn){
	var _types = {
		"select": "select"
	  , "radio": "radio"
		...
		async.eachSeries(this.columns(), function(col, done){
			...
			if(col.select()){
				...
				select.table.instance()
					.where(where)
					.sort({asc:true})
					.field(key, select.display)
					.all(function(err, rows){
						if( !err ){
							col.type = col.type && _types[col.type] || "select";
							col.extra = pair(rows, key, select.display);
							col.dict = dict(col.extra);
						}
						done(err);
					});
