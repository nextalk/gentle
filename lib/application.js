...
app.config = function( options ) {
	...
	var static = app.get('static path') || path.join( dir, 'public');

	app.use(express.favicon(path.join(static, 'favicon.ico')));
	app.use(brand);
	...
	app.use(express.cookieSession({
		key: app.get('session key') || 'gentle.sess'
	}));
...
app.configRoute = function() {
	var app = this
	  , upload = db.upload( app.get("upload") );
	app.locals.uploadPath = app.get("upload path") || "/upload/";
	if( app.get("auth") ) {
	...
	app.post("/:table/new.:format?", upload, db.create, route.create);

	app.get("/:table/:action.:format?", route.action(true));
	app.post("/:table/:action.:format?", upload, db.runAction, route.action(true));

	app.get("/:table/:id.:format?", route.edit);
	app.put("/:table/:id.:format?", upload, db.update, route.update);
	app.del("/:table/:id.:format?", db.del, route.del);

	app.get("/:table/:id/:action.:format?", route.action());
	app.post("/:table/:id/:action.:format?", upload, db.runAction, route.action());
}
